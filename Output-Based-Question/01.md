...
sayHi();
// It will throw error because sayHi() function is not accessable here
```
# 9
```js
let x = 1;
...
Why all 10 ?? what can we do to get 0,1,2
```js
function makeArmy() {
  let shooters = [];
...
// For every iteration new reference of J will be created.
```


# 10 what will be the value of i
```js
let i = 0;
...
function delay(f, ms) {
    return function () {
        setTimeout(() => f.apply(this, arguments), ms); // `apply` ensures the correct `this`
    };
}

let delayedGreet = delay(person.greet, 1000);
delayedGreet.call(person); // Use `call` to set `this` to `person`
